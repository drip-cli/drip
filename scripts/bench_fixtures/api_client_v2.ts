...
export interface ApiConfig {
  ...
  /** Value sent in the `user-agent` header on every request. Defaults to `drip-api-client/2`. */
  userAgent?: string;
}
...
export class ApiError extends Error {
  ...
  constructor(message: string, status: number, body: unknown, requestId: string) {
    ...
    const cap = (Error as { captureStackTrace?: (t: object, c: unknown) => void }).captureStackTrace;
    if (typeof cap === "function") cap(this, ApiError);
  }
...
const DEFAULT_USER_AGENT = "drip-api-client/2";
...
export class ApiClient {
  ...
  public async *paginate<T>(path: string, pageSize = 100, options: Omit<RequestOptions, "method" | "body"> = {}): AsyncGenerator<T[], void, void> {
    ...
    do {
      ...
      const page = await this.get<{ data: T[]; nextCursor?: string | null; hasMore?: boolean }>(path, { ...options, query });
      ...
      const advertisedHasMore = page.data.hasMore;
      ...
      if (advertisedHasMore === false && nextCursor !== null) {
        throw new ApiError("paginate: hasMore=false but nextCursor was non-null", page.status, page.data, page.requestId);
      }
      cursor = nextCursor;
    } while (cursor !== null);
  ...
  /**
   * Subscribe to a Server-Sent Events stream and yield each parsed event.
   *
   * The endpoint is expected to emit `data: <json>\n\n` framed payloads.
   * Lines beginning with `:` are treated as keep-alive comments and skipped.
   * The iterator terminates cleanly when the server closes the connection.
   */
  public async *streamSse<T>(path: string, options: Omit<RequestOptions, "method" | "body" | "parseAs"> = {}): AsyncGenerator<T, void, void> {
    const url = this.buildUrl(path, options.query);
    const headers = await this.composeHeaders({ ...(options.headers ?? {}), accept: "text/event-stream" });
    const ctrl = this.linkAbort(options.signal);
    try {
      const response = await fetch(url, { method: "GET", headers, signal: ctrl.signal });
      if (!response.ok || !response.body) await this.handleError(response, randomUUID());
    ...
    for (let attempt = 0; attempt <= maxAttempts; attempt++) {
      ...
      try {
        ...
        if (res.status >= 500 || res.status === 429) {
          ...
          const backoff = Math.min(cap, this.config.retryDelayMs * 1.7 ** attempt);
          const wait = Number.isFinite(retryAfter) && retryAfter > 0 ? retryAfter : backoff + Math.random() * 50;
        ...
        const wait = Math.min(cap, this.config.retryDelayMs * 1.7 ** attempt) + Math.random() * 50;
        await delay(Math.min(wait, 30_000));
      }
    }
    if (lastError) throw lastError;
  ...
  public async refreshToken(): Promise<string> {
    ...
    const priorToken = this.cachedToken;
    const priorExpiresAt = this.tokenExpiresAt;
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        ...
        if (priorToken !== null && this.cachedToken === priorToken) {
          throw new ApiError("token refresh produced an identical token", 0, null, randomUUID());
        }
        if (this.tokenExpiresAt <= Date.now()) {
          throw new ApiError("token refresh produced an already-expired token", 0, null, randomUUID());
        }
        ...
        const backoff = Math.min(5_000, this.config.retryDelayMs * 2 ** attempt) + Math.random() * 50;
        await delay(backoff);
      }
    }
    if (priorToken !== null && priorExpiresAt > Date.now()) {
      this.cachedToken = priorToken;
      this.tokenExpiresAt = priorExpiresAt;
    }
    if (lastError instanceof Error) throw lastError;
    throw new ApiError("token refresh failed for unknown reason", 0, null, randomUUID());
  }
   ...
   * fails so the caller still gets something to log. Captures a fresh stack
   * frame so the throw site is preserved across async boundaries.
   */
  protected async handleError(response: Response, requestId: string): Promise<never> {
    ...
    try {
      ...
      } else if (lowerType.startsWith("text/") || lowerType.includes("xml") || lowerType === "") {
        body = await response.text();
      } else {
        const buf = await response.arrayBuffer();
        body = `<binary ${lowerType} ${buf.byteLength} bytes>`;
      }
    } catch {
    ...
    if (typeof body === "object" && body !== null) {
      ...
      if (typeof rec.message === "string") {
        ...
        message = rec.error;
      } else if (typeof rec.error === "object" && rec.error !== null && "message" in rec.error) {
        message = String((rec.error as { message: unknown }).message);
      } else if (typeof rec.detail === "string") {
        message = rec.detail;
      } else {
        message = `HTTP ${response.status} ${response.statusText}`;
      }
    } else if (typeof body === "string" && body.length > 0 && body.length < 256) {
      message = body;
    } else {
      message = `HTTP ${response.status} ${response.statusText}`;
    }
    ...
    if (retryAfterRaw) {
      message = `${message} (retry-after=${retryAfterRaw})`;
    }
    const err = new ApiError(message, response.status, body, requestId);
    const cap = (Error as { captureStackTrace?: (t: object, c: unknown) => void }).captureStackTrace;
    if (typeof cap === "function") cap(err, this.handleError);
    throw err;
  }
  ...
  protected async composeHeaders(extra?: Record<string, string>): Promise<Record<string, string>> {
    ...
    if (!merged["user-agent"]) merged["user-agent"] = this.config.userAgent ?? DEFAULT_USER_AGENT;
