        ...
        while (pos_ < src_.size()) {
            ...
            if (c == '\\') {
                ...
                switch (e) {
                    ...
                    case 'u': {
                        if (pos_ + 4 > src_.size()) {
                            set_error(ParseErrorKind::InvalidEscape, "truncated \\u escape", line_, col_);
                        }
                        unsigned cp = 0;
                        for (int i = 0; i < 4; ++i) {
                            char h = src_[pos_++]; ++col_;
                            cp <<= 4;
                            if (h >= '0' && h <= '9') cp |= unsigned(h - '0');
                            else if (h >= 'a' && h <= 'f') cp |= unsigned(h - 'a' + 10);
                            else if (h >= 'A' && h <= 'F') cp |= unsigned(h - 'A' + 10);
                            else set_error(ParseErrorKind::InvalidEscape, "invalid hex in \\u", line_, col_);
                        }
                        if (cp < 0x80) {
                ...
                set_error(ParseErrorKind::InvalidEscape, "control char in string", line_, col_);
            } else {
