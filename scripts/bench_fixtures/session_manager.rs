...
const DEFAULT_ABSOLUTE_TIMEOUT_SECS: u64 = 28_800;
const DEFAULT_MAX_PER_USER: usize = 8;
const TOKEN_PREFIX: &str = "drp_v1_";
const ROTATION_GRACE_SECS: u64 = 300;
const GC_BATCH_SIZE: usize = 256;
...
impl SessionManager {
    ...
    pub fn garbage_collect(&self) -> Result<usize, SessionError> {
        ...
        let candidates: Vec<String> = guard
            .iter()
            .filter(|(_, s)| s.expires_at < now || s.state != SessionState::Active)
            .take(GC_BATCH_SIZE)
            .map(|(k, _)| k.clone())
            .collect();
    ...
    pub async fn validate_token(&self, token: &str) -> Result<Session, SessionError> {
        ...
        if token.len() < TOKEN_PREFIX.len() + 16 || token.len() > 256 {
            return Err(SessionError::TokenInvalid);
        }
    ...
    /// Render a Prometheus-flavored exposition of internal counters so the
    /// metrics scraper can pull them on its usual interval. We deliberately
    /// avoid pulling in the `prometheus` crate here because the production
    /// stack has its own registry that wraps these strings.
    pub fn export_metrics(&self) -> String {
        let active = self.count_active();
        let cache = self.cache.try_read();
        let total = cache.as_ref().map(|c| c.len()).unwrap_or(0);
        let revoked = cache
            .as_ref()
            .map(|c| c.values().filter(|s| s.state == SessionState::Revoked).count())
            .unwrap_or(0);
        let mut out = String::new();
        out.push_str(&format!("drip_sessions_active {}\n", active));
        out.push_str(&format!("drip_sessions_total {}\n", total));
        out.push_str(&format!("drip_sessions_revoked {}\n", revoked));
        out
    }
