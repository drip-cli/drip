...
{
    private const int MaxItemsPerOrder = 200;
    private const int DefaultPageSize = 50;
    private const decimal MaxOrderTotal = 1_000_000m;
    ...
    public async Task<Order> CreateAsync(CreateOrderRequest request, CancellationToken ct)
    {
        ...
        await PublishEventAsync(new { Type = "OrderCreated", order.Id, order.CustomerId, order.TotalAmount }, ct)
            .ConfigureAwait(false);
    ...
    /// <summary>
    /// Full-text-ish search across customer id and SKU. Backed by ILIKE on
    /// PostgreSQL; consider replacing with a dedicated search index for
    /// large catalogues.
    /// </summary>
    /// <param name="term">Free-text search term, case insensitive.</param>
    /// <param name="limit">Maximum rows to return.</param>
    /// <param name="ct">Cancellation token.</param>
    /// <returns>Matching orders ordered by recency.</returns>
    [Obsolete("Search has moved to OrderQueryHandler in the read-model project; will be removed in 3.0.")]
    public async Task<IReadOnlyList<Order>> SearchAsync(string term, int limit, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(term))
            return Array.Empty<Order>();
        if (term.Length > 128)
            throw new ArgumentException("Search term too long (max 128 chars).", nameof(term));

        var needle = term.Trim().ToLowerInvariant();
        if (needle.Length < 2)
            return Array.Empty<Order>();

        // Reject obvious SQL-injection-shaped inputs early; the parameterised
        // EF query would still be safe, but the database load isn't worth it.
        foreach (var ch in needle)
        {
            if (ch == ';' || ch == '%' || ch == '\\' || ch == '\'' || ch == '"')
                throw new ArgumentException($"Search term contains illegal character '{ch}'.", nameof(term));
        }

        var clamped = Math.Clamp(limit, 1, 200);
        if (Guid.TryParse(needle, out var asGuid))
        {
            var byId = await _db.Orders.AsNoTracking()
                .Include(o => o.Items)
                .Where(o => o.Id == asGuid || o.CustomerId == asGuid)
                .OrderByDescending(o => o.CreatedAt)
                .Take(clamped)
                .ToListAsync(ct)
                .ConfigureAwait(false);
        ...
        var rows = await _db.Orders.AsNoTracking()
            .Include(o => o.Items)
            .Where(o => o.Items.Any(i => i.Sku.ToLower().Contains(needle))
                     || o.CustomerId.ToString().Contains(needle))
            .OrderByDescending(o => o.CreatedAt)
            .Take(clamped)
            .ToListAsync(ct)
            .ConfigureAwait(false);

        _logger.LogDebug("SearchAsync({Term}) returned {Count} rows", needle, rows.Count);
        return rows;
    }
    ...
    private async Task ValidateAsync(CreateOrderRequest request, CancellationToken ct)
    {
        ...
        foreach (var line in request.Items)
        {
            if (string.IsNullOrWhiteSpace(line.Sku))
                throw new ArgumentException("Every line must specify a SKU.", nameof(request));
            if (line.Quantity <= 0)
            ...
            if (line.Discount < 0m || line.Discount > line.UnitPrice * line.Quantity)
    ...
    /// <summary>
    /// Publishes a domain event, swallowing transport errors and logging
    /// them. The outbox pattern is intentionally NOT used here in v1 —
    /// see v2 for the durable variant.
    /// </summary>
    /// <typeparam name="T">Event payload type.</typeparam>
