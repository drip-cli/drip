...
def batch_calculate(
    ...
    *,
    ...
    """Price a batch of orders, collecting per-order results.

    With ``fail_fast=True`` the first failure raises;
    otherwise each order returns a 3-tuple of
    ``(order, invoice_or_None, exception_or_None)`` so the
    caller can pick a per-order strategy (retry, requeue,
    log-and-continue).
    """
    results: List[Tuple[Order, Optional[Invoice], Optional[Exception]]] = []
    for order in orders:
        try:
            invoice = engine.calculate(order)
            results.append((order, invoice, None))
        except PricingError as exc:
            if fail_fast:
                raise
            logger.warning("order %s priced with error: %s", order.order_id, exc)
            results.append((order, None, exc))
    return results
...
class PricingEngine:
    ...
    def _cache_put(self, key: str, invoice: Invoice) -> None:
        ...
        if self._cache:
            expired = [
                k for k, (cached_at, _) in self._cache.items()
